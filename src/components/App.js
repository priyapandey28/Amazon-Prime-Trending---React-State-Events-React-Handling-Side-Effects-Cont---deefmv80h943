...
const App = (props) => {
  ...
  return (
    <React.Fragment>
      ...
      <button data-testid="button-next" onClick={slideNext} disabled={nextBtnDisable}>Next</button>
      <button data-testid="button-prev" onClick={slidePrev} disabled={prevBtnDisable}>Prev</button>
      <button data-testid="button-restart" onClick={restart} disabled={prevBtnDisable}>Restart</button>
    </React.Fragment>
  );
};
